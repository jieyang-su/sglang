...
class EagleDraftWorker(BaseDraftWorker):
    ...
    def draft(self, batch: ScheduleBatch):
        draft_input: EagleDraftInputV2 = batch.spec_info
    ...
    def _draft_extend_for_prefill(
        ...
        # Install draft-extend spec_info for the extend forward. Extend
        # input is transient (only hidden_states + shape info needed for the
        # forward); the next-iter spec_info constructed below carries
        # bonus_tokens for the subsequent draft step.
    ...
    def _draft_extend_for_decode(
        ...
        draft_extend_input.num_tokens_per_req = self.speculative_num_draft_tokens
    ...
    def forward_batch_generation(self, batch: ScheduleBatch, on_publish=None):
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            if batch.spec_info is None:
                ...
                batch.spec_info = EagleDraftInputV2.create_idle_input(
                    device=self.device,
            ...
            with (
                ...
                verify_input: EagleVerifyInputV2 = self.draft_worker.draft(batch)
            assert verify_input.is_verify_input()
            batch.spec_info = verify_input
            batch_output, verify_output = self.verify(batch)
            ...
            with (
                ...
                self.draft_worker._draft_extend_for_decode(
                    batch, batch_output, verify_output
                )
    ...
    def verify(
        ...
        if (
            ...
            self._mamba_verify_update(
                batch, accept_lens, verify_output.accept_indices, bs
            )
        ...
        if not batch.forward_mode.is_idle():
            ...
            fill_bonus_tokens[(bs,)](
                ...
                accept_lens,
                bonus_tokens,
                verify_output.accept_indices.shape[1],
            )
        else:
        ...
        if not batch.forward_mode.is_idle() and self.topk > 1:
            ...
            verify_output.predict = self._finalize_accept_tree_path(
                batch,
                verify_output.accept_indices,
                accept_lens,
                verify_output.predict,
        ...
        # verify_output is V2-internal: consumed by _draft_extend_for_decode in
        # the same forward_batch_generation call. Returning as tuple (not on
        # batch_result) keeps GenerationBatchResult scheduler-facing only.
